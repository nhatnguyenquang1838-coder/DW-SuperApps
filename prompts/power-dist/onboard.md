...
```text
Onboard DW SUPER Power Dist for the current target system and host environment.

Treat DW-SuperApps as the distribution and host-control workspace. Treat the selected system as the owner of runtime and project configuration only.
...
5. target-system instructions and selected Power manifests
...
- Keep `--target` as the system runtime target.
- Use `--store-root` only for tests or explicit external workspace layouts.
- Write runtime and configuration only under declared system roots such as `.gwc`, `.ua`, `.task-me`, `.bmad`, `_bmad`, and `_bmad-output`.
- Generate host adapters only in DW-SuperApps.
- Resolve installed package entrypoints before source submodules.
- Never create `<system>/.dw/powers` or Power host-skill payloads in the system.
- Detect existing `<system>/.dw/powers/<power-id>` as LEGACY_TARGET_INSTALL and preserve it.
- Split BMAD ownership: package and host skills in DW-SuperApps; project configuration/output in the target system.
...
Use:
./bin/dw power install <power-id> --source auto --target systems/<system-id>
