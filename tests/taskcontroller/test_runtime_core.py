...
# Explicit caller-supplied "now" for lease currentness/expiry decisions.
# Lease.1 in this module expires at 2026-08-20T01:00:00Z, so this value is
# BEFORE expiry (lease is current). Passed explicitly per the time invariant;
# no hidden/wall-clock time is used.
...
from taskcontroller.runtime.errors import (
    ConcurrentStateError,
    EventRejected,
    LeaseConflictError,
    RuntimeError,
    ConcurrentStateError as CSE,
)
from taskcontroller.runtime.runtime_state import (
    AttemptRecord,
    StreamWatermark,
...
from taskcontroller.runtime.store import (
    StateStore,
    InMemoryStateStore,
    SnapshotRecord,
    RuntimeRecord,
...
def team_run_state():
    return TeamRunState(
        ...
        artifact_refs=["out.1"],
        last_event_cursor=EventCursor(last_event_id="evt.0", sequence=0),
        checkpoint=None,
        plan_version="p1",
        run_version="r1",
...
class TestInMemoryStateStore:
    ...
    def test_cas_mismatch_raises(self, store, versioned_run):
        ...
        with pytest.raises(ConcurrentStateError):
            store.put_run(versioned_run, 999)  # expected 999, current 0
...
# ---------------------------------------------------------------------------
# event_router: dedupe, sequence, CAS
# ---------------------------------------------------------------------------
...
class TestEventRouterSequence:
    ...
    def test_out_of_order_sequence_rejected(self, event_router):
        ...
        store = router._store
        current = store.get_run("run.1")
        # first event seq=5, no idempotency_key to avoid dedupe interference
        first = AgentEvent(
            event_id="evt.1",
            run_id="run.1",
            node_id="n1",
...
class TestLeaseGrant:
    ...
    def test_grant_detaches_existing_active_lease(self, lease_manager, sample_work_lease):
        ...
        current = store.get_run("run.1")
        # grant second lease for same quad → first should be detached
        second = WorkLease(
            lease_id="lease.2",
            run_id="run.1",
            node_id="n1",
            execution_id="exec.1",
            attempt_id="att.1",
            holder=ProviderRef("prov.remote"),
            fencing_token="ft-2",
            granted_at="2026-08-13T00:01:00Z",
            expires_at="2026-08-13T02:00:00Z",
            status=LeaseStatus.ACTIVE.value,
        )
        result = mgr.grant(second, current.version, store.get_run("run.1"))
        # version: initial=0, grant1=1, grant2(detach+register)=3
        assert result.version == 3
        # first lease should now be RELEASED (detached)
