...
from taskcontroller.runtime.runtime_state import AttemptRecord, RuntimeLeaseState, RuntimeSnapshotMeta, StreamWatermark, VersionedRunState, make_attempt_record, make_versioned_run
...
def _make_lease(lease_id: str='lease.1', status: str=LeaseStatus.ACTIVE.value, expires_at: str=_EXPIRES, fencing_token: str=_FENCE, holder: ProviderRef | None=None) -> WorkLease:
    if holder is None:
        holder = ProviderRef(provider_id='provider.1')
    return WorkLease(lease_id=lease_id, run_id='run.1', node_id='n1', execution_id='exec.1', attempt_id='att.1', holder=holder, fencing_token=fencing_token, granted_at=_NOW, expires_at=expires_at, status=status)

def _make_state(run_status: str=RunStatus.RUNNING.value, node_status: str=NodeStatus.RUNNING.value, lease: WorkLease | None=None) -> VersionedRunState:
    run = TeamRunState(run_id='run.1', status=run_status, nodes={'n1': _make_node_state(node_status, lease.lease_id if lease else None)}, active_attempts=['att.1'], active_leases=[lease.lease_id if lease else 'lease.1'])
    meta = RuntimeSnapshotMeta(attempt_registry={'att.1': make_attempt_record(attempt_id='att.1', run_id='run.1', node_id='n1', execution_id='exec.1', fencing_token=_FENCE, current_attempt_number=1, current_lease_id=lease.lease_id if lease else None)}, leases=RuntimeLeaseState(leases={lease.lease_id: lease} if lease else {}), stream_watermarks={}, event_cursor=None, dedupe_fingerprints={}, journal_position=0)
...
def _make_node_state(status: str, lease_ref: str | None=None) -> 'NodeState':
    ...
    return NodeState(status=status, contract_ref='ctr.1', current_attempt=1, lease_ref=lease_ref, artifact_refs=[])
...
class TestGrantDetachesExistingActiveLease:
    ...
    def test_grant_detaches_existing_active_lease(self, seeded):
        ...
        existing = mgr.current('run.1', 'n1', 'exec.1', 'att.1', now=_NOW)
        ...
        new = mgr.current('run.1', 'n1', 'exec.1', 'att.1', now=_NOW)
        assert new is not None
        assert new.lease_id == 'lease.2'
        assert new.status == LeaseStatus.ACTIVE.value
        assert updated.version == current.version + 2
...
class TestRenew:
    ...
    def test_renew_extends_expiry_with_matching_fencing(self, seeded):
        ...
        updated = mgr.renew('lease.1', new_expires, _FENCE, current.version, current, now=_NOW)
        lease = mgr.current('run.1', 'n1', 'exec.1', 'att.1', now=_NOW)
        assert lease is not None
    ...
    def test_renew_rejects_wrong_fencing(self, seeded):
        ...
        with pytest.raises(LeaseConflictError, match='fencing_token mismatch'):
            mgr.renew('lease.1', _EXPIRES, 'wrong-fence', current.version, current, now=_NOW)

    def test_renew_rejects_non_active(self, seeded):
        mgr, store, current = seeded
        mgr.release('lease.1', current.version, current, now=_NOW)
        fresh = store.get_run('run.1')
        with pytest.raises(LeaseConflictError, match='cannot renew non-ACTIVE'):
            mgr.renew('lease.1', _EXPIRES, _FENCE, fresh.version, fresh, now=_NOW)
    ...
    def test_renew_rejects_expired(self, seeded_expired):
        ...
        with pytest.raises(LeaseConflictError, match='cannot renew expired'):
            mgr.renew('lease.1', _EXPIRES, _FENCE, current.version, current, now=_NOW)

class TestRelease:

    def test_release_transitions_active_to_released(self, seeded):
        mgr, store, current = seeded
        updated = mgr.release('lease.1', current.version, current, now=_NOW)
        lease = mgr.current('run.1', 'n1', 'exec.1', 'att.1', now=_NOW)
        assert lease is None
    ...
    def test_release_rejects_non_active(self, seeded):
        ...
        with pytest.raises(LeaseConflictError, match='cannot release non-ACTIVE'):
            mgr.release('lease.1', fresh.version, fresh, now=_NOW)
...
class TestExpire:
    ...
    def test_expire_transitions_node_to_lease_expired(self, seeded):
        mgr, store, current = seeded
        updated = mgr.expire('lease.1', current.version, current, now=_NOW)
    ...
    def test_expire_updates_attempt_record(self, seeded):
        mgr, store, current = seeded
        mgr.expire('lease.1', current.version, current, now=_NOW)
    ...
    def test_expire_current_lease_only(self, seeded):
        ...
        with pytest.raises(LeaseConflictError, match='cannot expire non-ACTIVE'):
            mgr.expire('lease.1', fresh.version, fresh, now=_NOW)
        fresh2 = store.get_run('run.1')
        mgr.expire('lease.2', fresh2.version, fresh2, now=_NOW)
        node = store.get_run('run.1').state.nodes['n1']
        assert node.status == NodeStatus.LEASE_EXPIRED.value
...
class TestOldLeaseOperationsCannotAffectCurrent:
    ...
    def test_old_lease_release_does_not_detach_current(self, seeded):
        ...
        with pytest.raises(LeaseConflictError, match='cannot release non-ACTIVE'):
            mgr.release('lease.1', fresh.version, fresh, now=_NOW)
        current_lease = mgr.current('run.1', 'n1', 'exec.1', 'att.1', now=_NOW)
...
def seeded_expired():
    ...
    _EXPIRED = '2026-08-14T09:00:00Z'
    _NOW = '2026-08-14T10:00:00Z'
    expired_lease = WorkLease(lease_id='lease.1', run_id='run.1', node_id='n1', execution_id='exec.1', attempt_id='att.1', holder=ProviderRef(provider_id='provider.1'), fencing_token='fence.1', granted_at=_NOW, expires_at=_EXPIRED, status=LeaseStatus.ACTIVE.value)
    state = VersionedRunState(state=TeamRunState(run_id='run.1', status='RUNNING', nodes={'n1': NodeState(status='RUNNING', contract_ref='ctr.1', current_attempt=1, lease_ref='lease.1', artifact_refs=[])}, active_attempts=['att.1'], active_leases=['lease.1']), version=1, meta=RuntimeSnapshotMeta(attempt_registry={'att.1': make_attempt_record(attempt_id='att.1', run_id='run.1', node_id='n1', execution_id='exec.1', fencing_token='fence.1', current_attempt_number=1, current_lease_id='lease.1')}, leases=RuntimeLeaseState(leases={'lease.1': expired_lease}), stream_watermarks={}, event_cursor=None, dedupe_fingerprints={}, journal_position=0))
