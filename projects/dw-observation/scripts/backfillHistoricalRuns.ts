 ...
 *
 * Reads authoritative repo evidence (fixtures + receipts) and builds the
 ...
 * report including a SCHEMA-AWARE constraint check (NOT NULL / CHECK enum /
 * FK) that mirrors the committed DDL, so DML-vs-DDL validity is proven
 * locally without a remote database.
 *
 * Remote apply is gated behind exact G6 approval (supabase db push --linked
 ...
 * value (RECONSTRUCTION_META.json / env DW_OBS_RECONSTRUCTED_AT).
 * Historical timestamps use real evidence (PR mergedAt, CI createdAt, issue
 * createdAt). No invented midnights.
 */
...
// Authoritative evidence (from GitHub PR/issue/CI, captured for this run) ---
const EVIDENCE = {
  issue: {
  ...
  // Scope/SHA receipts (from M3-M4-EVIDENCE.md / PR receipts)
  scopeHash: { "79": "aa5756f5dfc424ba" } as Record<string, string>,
  mergeSha: { "79": "a992fa4824db17434f6bdf8aabe8d6f435cc5767" } as Record<string, string>,
};
...
type SourceKind =
  ...
  | "ci_run" | "gwc_artifact" | "reconstruction" | "repo_governance";
...
interface RunsRow {
  ...
  scope_sha?: string; scope_hash?: string; base_sha?: string; head_sha?: string; merge_sha?: string;
...
// Per seq=11: attach fixtures as golden_fixture sources preserving source_run_id;
// do NOT treat synthetic fixture events as canonical run_events and do NOT
// fabricate a historical run id (no R1 / M3 / M4 runs).
function buildFixtureSources(): { sources: SourceRow[] } {
  ...
  sources.push({
    source_id: `golden-${tc.run_id}`,
    run_id: tc.run_id, // the canonical reconstructed M0 run id (R2)
  ...
  sources.push({
    source_id: `golden-${gwc.run_id}`,
    // attach to the canonical reconstructed M0 run; preserve GWC source_run_id in evidence
    run_id: tc.run_id,
    source_system: "gwc",
    source_event_id: undefined,
    source_kind: "golden_fixture",
...
interface ReconDef {
  run_id: string; pr_number: number; issue: string; authority_ref?: string;
  gate_id?: string; nodes?: string[]; ci_run_id?: string; milestone: string;
  scope_hash?: string; base_sha?: string; head_sha?: string; merge_sha?: string;
  branch?: string; has_alignment_scope: boolean;
}
const RECONSTRUCTED: ReconDef[] = [
  {
    run_id: "DW-OBS-M0-20260821-R2", pr_number: 76, issue: "71",
    authority_ref: "G2-DW-OBS-M0-20260821-R2", gate_id: "G2-DW-OBS-M0-20260821-R2",
    nodes: ["71"], milestone: "M0", has_alignment_scope: true,
    base_sha: EVIDENCE.pr["76"].base, head_sha: EVIDENCE.pr["76"].head,
    branch: "auto/SCRUM-555-dw-observation-m0-r2",
  },
  {
    run_id: "G2-DW-OBS-M1-20260821-R1", pr_number: 77, issue: "72",
    authority_ref: "G2-DW-OBS-M1-20260821-R1", gate_id: "G2-DW-OBS-M1-20260821-R1",
    milestone: "M1", has_alignment_scope: true,
    base_sha: EVIDENCE.pr["77"].base, head_sha: EVIDENCE.pr["77"].head,
    branch: "auto/SCRUM-555-dw-observation-m1-r1",
  },
  {
    run_id: "DW-OBS-M2-20260822-R1", pr_number: 78, issue: "73",
    milestone: "M2", has_alignment_scope: true,
    base_sha: EVIDENCE.pr["78"].base, head_sha: EVIDENCE.pr["78"].head,
    branch: "auto/SCRUM-555-dw-observation-m2-r1",
  },
  {
    run_id: "DW-OBS-M3M4-20260823-R1", pr_number: 79, issue: "70",
    authority_ref: "G2-DW-OBS-M3M4-20260823-R1", gate_id: "G2-DW-OBS-M3M4-20260823-R1",
    nodes: ["74", "75"], ci_run_id: "32589399526", milestone: "M3/M4", has_alignment_scope: true,
    scope_hash: EVIDENCE.scopeHash["79"], base_sha: EVIDENCE.pr["79"].base,
    head_sha: EVIDENCE.pr["79"].head, merge_sha: EVIDENCE.mergeSha["79"],
...
function buildReconstructed(): { runs: RunsRow[]; gates: GateRow[]; nodes: NodeRow[]; sources: SourceRow[]; artifacts: ArtifactRow[] } {
  ...
  for (const d of RECONSTRUCTED) {
    ...
    runs.push({
      ...
      jira_key: "SCRUM-555", parent_issue: d.issue, authority_ref: d.authority_ref,
      scope_sha: d.head_sha, scope_hash: d.scope_hash, base_sha: d.base_sha,
      head_sha: d.head_sha, merge_sha: d.merge_sha, base_branch: "pre-prod",
      branch: d.branch, pr_number: d.pr_number, ci_run_id: d.ci_run_id,
    ...
    sources.push({
      source_id: `pr-${d.run_id}`, run_id: d.run_id, source_system: "github",
      source_kind: "github_pr", capture_provenance_verified: true,
      source_ref: `github:pull/${d.pr_number}`, occurred_at: mergedAt,
      evidence_refs: [`PR #${d.pr_number} merged ${mergedAt}`],
    });
    sources.push({
      source_id: `issue-${d.run_id}`, run_id: d.run_id, source_system: "github",
      source_kind: "github_issue", capture_provenance_verified: true,
      source_ref: `github:issue/${d.issue}`, occurred_at: iss,
      evidence_refs: [`issue #${d.issue} created ${iss}`],
    });
    sources.push({
      source_id: `recon-${d.run_id}`, run_id: d.run_id, source_system: "repo_governance",
      source_kind: "reconstruction", capture_provenance_verified: false,
      source_ref: ".gwc/tasks/SCRUM-555/history-backfill/RECONSTRUCTION.md",
      occurred_at: RECONSTRUCTED_AT, evidence_refs: [`PR #${d.pr_number}`, `issue #${d.issue}`],
    });
    if (ci) {
      sources.push({
        source_id: `ci-${d.run_id}`, run_id: d.run_id, source_system: "github",
        source_kind: "ci_run", capture_provenance_verified: true,
        source_ref: `github:actions:run/${d.ci_run_id}`, occurred_at: ci.created,
    ...
    if (d.gate_id) {
      gates.push({
        gate_id: d.gate_id, run_id: d.run_id, gate_label: d.gate_id,
        authority_ref: d.authority_ref, state: "passed",
        summary: `G2 authority for ${d.run_id}`,
    ...
    for (const n of d.nodes ?? []) {
      nodes.push({
        node_id: n, run_id: d.run_id, gate_id: d.gate_id,
        state: "done", label: `node ${n}`,
        source_refs: [`github:issue/${d.issue}`],
      });
    }

    // Artifacts: context -> issue ts; delivery -> PR merge ts; alignment -> PR merge ts.
    const baseArts: Array<Pick<ArtifactRow, "artifact_type" | "reconstruction_basis" | "source_occurred_at" | "source_refs" | "confidence" | "evidence_quality">> = [
      {
        ...
        source_occurred_at: iss, source_refs: [`github:issue/${d.issue}`],
        confidence: "HIGH", evidence_quality: "STRONG",
      },
      {
        ...
        source_occurred_at: mergedAt, source_refs: [`github:pull/${d.pr_number}`],
        confidence: "HIGH", evidence_quality: "STRONG",
      },
    ];
    if (d.has_alignment_scope) {
      baseArts.push({
        ...
        source_occurred_at: mergedAt, source_refs: [`github:pull/${d.pr_number}`, `github:issue/${d.issue}`],
        confidence: "HIGH", evidence_quality: "STRONG",
      });
    }
    // CI receipt: real if present, else missing_unreconstructable (NOT a merged-PR artifact)
    if (ci) {
      baseArts.push({
        ...
        source_occurred_at: ci.created, source_refs: [`github:actions:run/${d.ci_run_id}`, `github:pull/${d.pr_number}`],
        confidence: "HIGH", evidence_quality: "STRONG",
      });
    } else {
      baseArts.push({
        artifact_type: "reconstructed_ci_evidence",
        reconstruction_basis: "No CI run receipt captured; recorded as missing, not inferred from merged PR",
        source_occurred_at: undefined, source_refs: [`github:pull/${d.pr_number}`, `github:issue/${d.issue}`],
        confidence: "UNKNOWN", evidence_quality: "NONE",
      });
    }

    for (const a of baseArts) {
      const isMissing = a.artifact_type === "reconstructed_ci_evidence" && !ci;
      artifacts.push({
        artifact_id: `${d.run_id}:${a.artifact_type}`,
        run_id: d.run_id, artifact_type: a.artifact_type,
        artifact_status: isMissing ? "missing_unreconstructable" : "reconstructed",
        original_artifact_present: false,
        source_occurred_at: a.source_occurred_at,
        effective_at: mergedAt,
        reconstructed_at: RECONSTRUCTED_AT,
        reconstruction_basis: a.reconstruction_basis,
        source_refs: a.source_refs, confidence: a.confidence, evidence_quality: a.evidence_quality,
        reconstructed_by: "TaskController/Hermes", payload: { milestone: d.milestone },
      });
    }
    ...
    if (d.run_id === "DW-OBS-M3M4-20260823-R1") {
      sources.push({
        source_id: `evidence-DW-OBS-M3M4-20260823-R1`, run_id: d.run_id,
        source_system: "repo_governance", source_kind: "repo_governance",
        capture_provenance_verified: true, source_ref: evidencePath,
        source_digest: evidenceDigest, occurred_at: undefined,
        evidence_refs: [evidencePath],
      });
      artifacts.push({
        artifact_id: `${d.run_id}:M3-M4-EVIDENCE.md`,
        run_id: d.run_id, artifact_type: "historical_evidence_file",
        artifact_status: "original", original_artifact_present: true,
        source_occurred_at: undefined, // file creation unknown
        effective_at: mergedAt, // existence proven at PR #79 merge
...
function validate(
  ...
  const SRC_KIND = ["live_capture","golden_fixture","github_pr","github_issue","ci_run","gwc_artifact","reconstruction","repo_governance"];
...
function emitDML(runs: RunsRow[], events: any[], gates: GateRow[], nodes: NodeRow[], sources: SourceRow[], artifacts: ArtifactRow[]): string {
  ...
  L.push(`BEGIN;`);
  for (const r of runs) {
    L.push(`INSERT INTO runs (run_id,run_kind,source_system,epic_id,jira_key,parent_issue,authority_ref,scope_sha,scope_hash,base_sha,head_sha,merge_sha,base_branch,branch,pr_number,ci_run_id,ci_status,started_at,completed_at,reconstruction_basis,source_refs,confidence,evidence_quality,reconstructed_by,reconstructed_at,payload) VALUES (` +
      [sqlStr(r.run_id), sqlStr(r.run_kind), sqlStr(r.source_system), sqlStr(r.epic_id), sqlStr(r.jira_key), sqlStr(r.parent_issue), sqlStr(r.authority_ref), sqlStr(r.scope_sha), sqlStr(r.scope_hash), sqlStr(r.base_sha), sqlStr(r.head_sha), sqlStr(r.merge_sha), sqlStr(r.base_branch), sqlStr(r.branch), sqlStr(r.pr_number), sqlStr(r.ci_run_id), sqlStr(r.ci_status), sqlStr(r.started_at), sqlStr(r.completed_at), sqlStr(r.reconstruction_basis), sqlArr(r.source_refs), sqlStr(r.confidence), sqlStr(r.evidence_quality), sqlStr(r.reconstructed_by), sqlStr(r.reconstructed_at), `'{}'::jsonb`].join(",") +
...
function main() {
  ...
  // events = 0: golden fixture events are NOT canonical live events (seq=11).
  ...
  const out = {
    ...
    dry_run: true, remote_apply: false,
    ...
    reconstructed_at: RECONSTRUCTED_AT, ...report,
    runs: allRuns,
