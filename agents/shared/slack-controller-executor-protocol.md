...
Required fields:
- Human owner / watcher
- Gate journey G0 → G6 with current gate highlighted when GWC is active
...
Report immediately for scope drift, authority drift, evidence conflict, blocker/failure, or a material finding that invalidates the current plan.

Remain silent for chain-of-thought, tool invocation chatter, individual file reads/edits, raw terminal/test/CI output, repetitive CI polling, recovered transient retries, and low-level successful operations without semantic impact.
